...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      if (token.startsWith('driver-token-')) {
        const dId = token.replace('driver-token-', '');
  ...
  const handleDriverSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const term = driverIdOrEmail.trim().toLowerCase();
      
      const match = drivers.find((d: any) => 
        String(d.id) === term || 
        d.email.toLowerCase() === term || 
        d.phone.replace(/[\s+]/g, '').includes(term)
      );

      if (!match) {
        throw new Error('Driver profile not found. Please register or check details.');
      }

      setToken(`driver-token-${match.id}`);
      setUser({ id: match.id, name: match.name, role: 'Driver', email: match.email });
      router.push(`/drivers/${match.id}`);
    } catch (err: any) {
      console.error(err);
  ...
  const handleDriverRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newDriver = await api.post('/drivers', payload);
      setToken(`driver-token-${newDriver.id}`);
      setUser({ id: newDriver.id, name: newDriver.name, role: 'Driver', email: newDriver.email });
