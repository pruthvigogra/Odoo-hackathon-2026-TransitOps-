...
export default function Navbar() {
  ...
  const isFinancialAnalyst = ['Financial Analyst', 'Local Analyst', 'Senior Analyst'].includes(user?.role);
  ...
  const navItems = isDriver ? [
    ...
    { name: 'Safety Dashboard', href: '/safety', icon: LayoutDashboard },
    { name: 'Driver Compliance', href: '/drivers', icon: Users },
    { name: 'Trip Oversight', href: '/trips', icon: Navigation },
    { name: 'Alerts & Notifications', href: '/alerts', icon: AlertCircle, badgeKey: 'alerts' },
    { name: 'Settings', href: '/settings', icon: Settings },
  ] : isDispatcher ? [
    { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
    { name: 'Trip Dispatch', href: '/trips', icon: Navigation },
    ...
    { name: 'Finance Dashboard', href: '/finance', icon: LayoutDashboard },
    { name: 'Fuel & Expenses', href: '/fuel', icon: Fuel },
    { name: 'Fleet Overview', href: '/vehicles', icon: Truck },
    { name: 'Settings', href: '/settings', icon: Settings },
  ] : [
    { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
    { name: 'Vehicle Management', href: '/vehicles', icon: Truck },
  ...
  const getHomeRoute = () => {
    if (isDriver) return `/drivers/${user.id}`;
    if (isSafetyOfficer) return '/safety';
    if (isFinancialAnalyst) return '/finance';
    return '/dashboard';
  };
  ...
  return (
    <aside className="w-16 md:w-64 h-screen bg-white/90 backdrop-blur-md border-r border-slate-200 sticky top-0 flex flex-col py-6 px-3 md:px-4 shadow-sm z-30 transition-all duration-300">
      ...
      <div className="flex-shrink-0">
        <Link href={getHomeRoute()} className="flex items-center gap-3 select-none group px-1 md:px-2">
