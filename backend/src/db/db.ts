...
export async function setupDatabase() {
  ...
  const demoUsers = [
    ['admin@transitops.com', 'admin123', 'Fleet Administrator', 'Admin'],
    ['manager@transitops.com', 'admin123', 'Fleet Manager', 'Admin'],
    ['fleet.manager@transitops.demo', 'FleetDemo@123', 'Fleet Manager Demo', 'Fleet Manager'],
    ['dispatcher@transitops.demo', 'DispatchDemo@123', 'Dispatcher Demo', 'Dispatcher'],
    ['safety.officer@transitops.demo', 'SafetyDemo@123', 'Safety Officer Demo', 'Safety Officer'],
    ['finance.analyst@transitops.demo', 'FinanceDemo@123', 'Financial Analyst Demo', 'Financial Analyst']
  ];

  for (const u of demoUsers) {
    const existing = await query(`SELECT id FROM users WHERE email = $1`, [u[0]]);
    if (existing.rows.length === 0) {
      const hashedPw = await bcrypt.hash(u[1], 10);
      await query(`INSERT INTO users (email, password, name, role) VALUES ($1, $2, $3, $4)`, [u[0], hashedPw, u[2], u[3]]);
    }
  }
